...
__attribute__((section(".sdram_data"))) uint8_t pImageCroppedGray[320*320*3];
...
constexpr int kTensorArenaSize = 301 * 1024;
...
int main(void)
{
  ...
  LIB_IMAGE_InitStruct(&cropped, (uint8_t*)pImageCropped, 32, 32, IMAGE_FORMAT_RGB565);
  LIB_IMAGE_InitStruct(&croppedGray, (uint8_t*)pImageCroppedGray, 32, 32, IMAGE_FORMAT_RGB888);
  LIB_SW_Init(&sw);
  BSP_SDRAM_Init();
  ...
  while (1)
  {
	...
	if (!LIB_OV5640_CaptureSnapshot(&img, 5000))
	{
		...
		uint32_t j = 32*32, i = 0;
		for (i = 0; i < 32*32; ++i)
		{
			croppedGray.pData[j] = croppedGray.pData[i];
			croppedGray.pData[j + 32*32] = croppedGray.pData[i];
			j++;
		}
		...
		// RUN INFERENCE
		memcpy(input->data.uint8, (void*)croppedGray.pData, croppedGray.size);
