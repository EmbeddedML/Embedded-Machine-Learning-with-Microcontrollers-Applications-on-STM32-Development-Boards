...
model = tf.keras.models.load_model(os.path.join(KERAS_MODEL_DIR, "har_perceptron.h5"))
converter = tf.lite.TFLiteConverter.from_keras_model(model)
tflite_model = converter.convert()
tflite_save_path = os.path.join(TFLITE_MODEL_DIR, "har_perceptron.tflite")
with open(tflite_save_path, "wb") as tflite_file:
    tflite_file.write(tflite_model)

tflite_export_path = os.path.join(TFLITE_EXPORT_DIR, "har_perceptron")
