...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
#define BUFFER_SIZE		(2400*4)
int16_t AudioBuffer[BUFFER_SIZE] 		= {0}; 		// 16KHz Stereo
float AudioBufferF32Down[BUFFER_SIZE/4] = {0}; 		// 8KHz mono
float features[BUFFER_SIZE] 			= {0};
...
int main(void)
{
  ...
  while (1)
  {
	...
	if (LIB_AUDIO_PollForRecording(5000) == 0)
	{
		...
		{
			AudioBufferF32Down[i] = (float32_t)AudioBuffer[j]/(float32_t)max;
			j = j + 4;
		}
		ks_mfcc_extract_features(AudioBufferF32Down, features);

		/*SEND AUDIO DATA TO PC*/
		LIB_SERIAL_Transmit(features, BUFFER_SIZE, TYPE_F32);

		/*RUN INFERENCE*/
//		memcpy(input->data.int8, (void*)&AudioBuffer, BUFFER_SIZE/4);
//		LIB_MODEL_Run(&output);
//
//		/*SEND INFERENCE RESULTS TO PC*/
//		LIB_SERIAL_Transmit(output->data.int8, 10, TYPE_S8);
	}
  }
  /* USER CODE END 3 */
}
